...
This is standard for every device and pre-builded into application process. Including:
- Device Name UI - Choose name for your new device or keep default. Call to [PUT Update a Device](https://iotapps.docs.apiary.io/#reference/devices/manage-single-device/update-a-device) API to update `device.desc` (name).
- Device Goals - if connected device type has goals (e.g. behaviors or scenarios) show the list of available scenarios. To get scenarios call to GET Device Goals by Type API. By selecting goal call to PUT Update a Device API to set device.goalId.
- Spaces - 
